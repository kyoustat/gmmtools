...
#' 
#' @references
#' \insertRef{you_l_2_2019}{gmmtools}
#' 
#' @concept diss
#' @export
dissL2 <- function(gmmobj1, gmmobj2){
  ## PREPARE
  check_gmmobj(gmmobj1,"dissL2")
  check_gmmobj(gmmobj2,"dissL2")
  if (ncol(gmmobj1$mean)!=ncol(gmmobj2$mean)){
    stop("* dissL2 : two gmm objects are not of same dimension.")
  }
  
  ## COMPUTE WITH C++
  output = as.double(cpp_gmmdist_l2(gmmobj1$weight, gmmobj1$mean, gmmobj1$variance, 
