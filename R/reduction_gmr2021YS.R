#' GMM Reduction by You (2021) via Iterative Update
#' 
...
#' @concept reduction
#' @export
gmr2021YS <- function(gmmobj, M=2, merger=c("mmm","wass2"), theta=1){
  ## INPUTS
  check_gmmobj(gmmobj,"gmr2021YS")
  myk      = max(1, round(M))
  if (myk >= length(as.vector(gmmobj$weight))){
    k.given = length(as.vector(gmmobj$weight))
    stop(paste0("* gmr2021YS : input 'M' should be smaller than ",k.given," with the given object.."))
  }
  ...
  if (myk < 2){ # single component : collapse via moment-preserving merge
    ...
    cpprun = cpp_reduction_2021YS(gmmobj$weight, gmmobj$mean, gmmobj$variance, myk, mytheta, mymerger)
