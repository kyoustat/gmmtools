...
#' 
#' @references 
#' \insertRef{runnalls_kullback-leibler_2007-1}{gmmtools}
#' 
#' @concept reduction
#' @export
gmr2007R <- function(gmmobj, M=2){
  ## INPUTS
  check_gmmobj(gmmobj,"gmr2007R")
  myk      = max(1, round(M))
  ...
  if (myk < 2){ # single component : collapse via moment-preserving merge
    ...
    cpprun = cpp_reduction_2007R(gmmobj$weight, gmmobj$mean, gmmobj$variance, myk)
